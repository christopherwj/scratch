...
# --- Settings ---
ticker = 'TSLA'
period = '2y'  # Changed to 2 years
interval = '1d'
min_trades_required = 1  # accept results with at least 1 trade
log_file = f"{ticker}_vcp_param_log.csv"

# --- Cleanup old cache files not matching current period & interval ---
def cleanup_old_cache(ticker, current_period, interval):
    pattern = f"{ticker}_*_*.csv"
    for file in glob.glob(pattern):
        if f"_{current_period}_" not in file or f"_{interval}." not in file:
            print(f"🧹 Removing old cache file: {file}")
            os.remove(file)

# --- Download or load cached data ---
def download_or_cache_data(ticker, period='2y', interval='1d'):
    filename = f"{ticker}_{period}_{interval}.csv"
    if os.path.exists(filename):
        print(f"📁 Loading cached data: {filename}")
        df = pd.read_csv(filename, index_col=0, parse_dates=True)
    else:
        print(f"🌐 Downloading {ticker} from yFinance for period {period}...")
...
# --- Detect VCP trades ---
def detect_vcp_trades(df, max_contraction_pct, atr_percentile_thresh, breakout_volume_ratio, window=7, max_hold=10):
    df = df.copy()
    ...
    for i in range(window, len(df) - 1):
        ...
        if not position:
            ...
            if next_price <= stop_price or hold_period >= max_hold:
...
param_space = {
    'max_contraction_pct': [50, 55, 60],
    'atr_percentile_thresh': [60, 70, 80],
    'breakout_volume_ratio': [0.7, 0.8, 1.0],
}
...
def evaluate_params(df):
    ...
    for max_contraction_pct, atr_thresh, vol_ratio in product(
        ...
        trades = detect_vcp_trades(df, max_contraction_pct, atr_thresh, vol_ratio, window=7, max_hold=10)
        n = len(trades)

        print(f"Params: max_contr={max_contraction_pct}, atr_thresh={atr_thresh}, vol_ratio={vol_ratio} -> trades={n}")
...
if __name__ == '__main__':
    cleanup_old_cache(ticker, period, interval)
