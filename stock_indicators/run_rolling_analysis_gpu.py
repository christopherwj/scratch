...
from src.rolling_optimizer_gpu import RollingOptimizerGPU
...
def visualize_performance_and_indicators(portfolio_value, price_data, trades, indicators, benchmark_data, title):
    ...
    # Save the figure
    plt.savefig('trading_analysis_results_gpu.png', dpi=300)
    print("\nChart saved to trading_analysis_results_gpu.png")
    plt.show()


def save_results_summary(portfolio_value, trades, params_history, stock_data, benchmark_data, initial_cash, ticker, benchmark_ticker, total_time):
    """
    Saves a human-readable summary of the analysis results and a detailed trade log.
    """
    # 1. Save trades log to CSV
    trades_path = 'trades_log_gpu.csv'
    ...
    summary = []
    summary.append("="*80)
    summary.append("GPU-ACCELERATED ADAPTIVE STRATEGY PERFORMANCE ANALYSIS")
    summary.append("="*80)
    summary.append(f"\n--- Performance Summary ({portfolio_value.index.min().date()} to {portfolio_value.index.max().date()}) ---")
    summary.append(f"Total processing time: {total_time/60:.2f} minutes")
    summary.append(f"GPU acceleration: Enabled (RTX 3090 Ti)")
    ...
    # 4. Write to file
    summary_path = 'results_summary_gpu.txt'
    with open(summary_path, 'w') as f:
        f.write("\n".join(summary))
    print(f"Human-readable results summary saved to {summary_path}")


def run_rolling_analysis_gpu(ticker, spy_ticker, start_date, end_date, train_window, test_window, num_cores, batch_size):
    """
    Main function to run the GPU-accelerated rolling optimization analysis and visualize the results.
    """
    global TICKER
    TICKER = ticker
    ...
    print(f"\nStarting GPU-accelerated optimization with a constrained grid of {valid_combos} valid combinations.")
    print(f"Using {num_cores} CPU cores and batch size of {batch_size} for GPU processing.")
    print("GPU acceleration enabled for indicator calculations and batch processing.")
    
    # --- 3. Run GPU-Accelerated Rolling Optimizer ---
    ...
    visualize_performance_and_indicators(
        ...
        benchmark_data=benchmark_data['Close'] if benchmark_data is not None else None,
        title=f"GPU-Accelerated Adaptive Strategy Analysis for {ticker}"
    )

    # --- 8. Print Summary to Console ---
    print("\n\n" + "="*80)
    print("FINAL SUMMARY")
    with open('results_summary_gpu.txt', 'r') as f:
        print(f.read())
    print("="*80)
...
if __name__ == '__main__':
    ...
    END_DATE = '2023-12-31'
    NUM_CORES = 16  # CPU cores for parallel processing
    BATCH_SIZE = 100  # GPU batch size for processing parameter combinations
    ...
    print("="*80)
    print("GPU-ACCELERATED ROLLING ANALYSIS")
    print("="*80)
